...
- [x] 웹 관리자 페이지(`sqladmin`) 및 원본 로그 시스템 구축 (인증 세션 미들웨어, 관리자 식별자 직접 비교 및 비밀번호 해싱 적용 완료)
- [x] 루트 엔드포인트 반환 형식 수정 (`set` -> `dict`)
- [x] 리포지토리 레이어에서 `db.commit()` 제거 및 서비스 레이어 트랜잭션 관리 적용 (원자성 보장)
- [x] `CongestionData` 및 `RawScannerData` 모델에 인덱스 및 유니크 제약 조건 추가 (성능 및 무결성 강화)
- [x] `CongestionRepository.upsert` 메서드에 원자적 Upsert (`ON CONFLICT`) 적용 (Race Condition 해결)
- [x] 시드 스크립트(`seed.py`)의 파괴적 초기화 로직을 함수 내부로 이동하여 안정성 강화
...
---
*마지막 업데이트: 2026-05-25 (리포지토리 트랜잭션 관리 수정)*
